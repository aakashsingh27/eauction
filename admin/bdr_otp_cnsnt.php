...
<li class="breadcrumb-item active" style="font-size: 18px;font-weight: bold;color: black !important;">Bidder login OTP consent</li>
</ol>
<div class="container-fluid">
...
<label class="form-label" style="font-size:16px !important;">Bidder OTP consent <span style="font-weight:bold;">(<?php echo $cncnst;?>)</span></label>
<select name="cnsnt"  class="form-control" style="border:1px solid black !important;" required>
<option value="">Choose consent</option>
<option value="otp">Login with otp</option>
<option value="without otp">Without OTP</option>
</select>
</div>
...
$ad_prdt="update otp_consent set
consent='$ttel' where id='1'";

$qst_ad_prdt=$db->query($ad_prdt);

if($qst_ad_prdt)
{
echo "<script>window.alert('Consent updated Successfully');window.location='';</script>";
