...
<?php 
$clt_sel_notc_sts="select * from `create_sale` where `create_by_liquiditor_id`='$a_idchk' and `End_Date_Auction` >='$timestamp' and `Start_Date_Auction` < '$timestamp'";
$qst_clt_sel_notc_sts=$db->query($clt_sel_notc_sts);
$sel_notc_nbr_rws=mysqli_num_rows($qst_clt_sel_notc_sts);
...
if($sel_notc_nbr_rws==0)
{
    ?>
    <center><span style="font-size: 25px;font-weight: bold;color: red;">No Bid is Live</span></center>
    <?php 
}
?>
