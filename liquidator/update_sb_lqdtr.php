...
$clt_dplc_sb_lqdtr="select * from sub_liquidator where `email`='$emsnl' and id='$sub_lqdtr_id'";
$qst_clt_dplc_sb_lqdtr=$db->query($clt_dplc_sb_lqdtr);
$clct_cnt=mysqli_num_rows($qst_clt_dplc_sb_lqdtr);
if($clct_cnt==1)
{
    ...
    if($result)
    {
       ...
       echo "<script>alert('Sub liquidator updated successfully');
...
{
    $clt_dplsc_sb_lqdtr="select * from sub_liquidator where `email`='$emsnl' and id!='$sub_lqdtr_id'";
$qst_clt_dplcs_sb_lqdtr=$db->query($clt_dplsc_sb_lqdtr);
$clctss_cnt=mysqli_num_rows($qst_clt_dplcs_sb_lqdtr);
if($clctss_cnt==0)
{
     $sql="update `sub_liquidator` set 
    `name`='$name',
    ...
    if($result)
    {
       ...
       echo "<script>alert('Sub liquidator updated successfully');
         window.location.href='vww_sb_lqdtr.php';
       </script>";
      // echo "check";
...
{
    echo "<script>window.alert('This email is already exist.');window.location='';</script>";
}
}
}
