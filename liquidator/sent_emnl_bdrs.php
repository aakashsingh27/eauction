...
{
$bdr_id=$_GET['id'];
$lqdtr_id=$_GET['lqdtr_id'];
    
    $clt_bdr_dtl="select * from newbidder_login where bidder_add_by_liquiditor_id='$lqdtr_id' and id='$bdr_id'";
    $qst_clt_bdr_dtl=$db->query($clt_bdr_dtl);
    $clct_clt_bdr_dtl=$qst_clt_bdr_dtl->fetch_assoc();

$bdr_emmnl=$clct_clt_bdr_dtl['Bidder_Email'];
$bdr_uid=$clct_clt_bdr_dtl['uid'];
$bdr_pswd=$clct_clt_bdr_dtl['Bidder_Password'];
...
$to = "$bdr_emmnl";                                                        
$subject = "IP Support (Login Credentials)--Date-time ($crt_det_tme)";
$txt = "<p3>Dear Ma’am/Sir,</p3><br><br>
<p3>Greeting of the day !!</p3><br><br>
<p3>Your Credentials are give below please check and login</p3><br><br>
<p3> <a href='https://eauction.ipsupport.in/bidder_login.php'>click here to login </a></p3><br>
<p3><b>Username :- </b> $bdr_uid</p3><br><br>
<p3><b>Password :- </b>$bdr_pswd</p3><br><br>
...
if(mail($to,$subject,$txt,$headers))
{
    ...
    $upd_emnl_sts="update newbidder_login set email_sent_status='send' where id='$bdr_id'";
    $qst_upd_emnl_sts=$db->query($upd_emnl_sts);
    if($qst_upd_emnl_sts)
    {
    ?>
    
    <script>window.alert('Bidder mail send successfully');window.location='vw_bidders.php';</script>
    <?php
}
}
//email end
